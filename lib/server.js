...
function Server(config) {
  ...
  /** @type {RedisClient} */
  this._redisClientDown = this._createRedisClientDown(configOptions.redisHost);
  /** @type {RedisClient} */
  this._redisClientUp = this._createRedisClientUp(configOptions.redisHost);
  /** @type {StatusServer} */
  this._statusServer = this._createStatusServer(configOptions.statusPort, configOptions.statusSecret);

  var self = this;
  configOptions.socketPorts.forEach(function(port) {
    self._createWorker(port);
  });
}

Server.prototype._createWorker = function(port) {
  var worker = childProcess.fork(path.join(__dirname, 'worker-fork.js'), ['-c=' + this._config.getPath(), '--socket-port=' + port]);
  this.addWorker(worker);
  var self = this;
  worker.on('message', function(event) {
    self.triggerEventUp(event.type, event.data);
  });
  worker.once('exit', function() {
    self._onWorkerExit(worker, port);
  });
};

Server.prototype._onWorkerExit = function(worker, port) {
  console.error('Worker `' + worker.pid + '` exited');
  if (this._isListening) {
    this.removeWorker(worker);
    this._createWorker(port);
  }
};
